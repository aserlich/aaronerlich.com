...
Output: `_generated/cv_publications.tex` — a fragment you can either
paste directly into the Overleaf project (replacing the existing
publication-section blocks) or `\\input{cv_publications}` from main.tex.
...
def main() -> int:
    ...
    out_parts.append(
        ...
        "% Paste or \\input{} this into Overleaf. Non-publication\n"
        "% sections (appointments, education, grants, etc.) are not\n"
        "% covered by this generator and stay hand-maintained.\n"
        "% ----------------------------------------------------------\n"
    )
